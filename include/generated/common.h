...
template<
    uint32_t Envelope_info_packet_InfoMessagePacket_name_LENGTH
>
class Envelope final: public ::EmbeddedProto::MessageInterface
{
  public:
    Envelope() = default;
    ...
    Envelope(const Envelope&& rhs ) noexcept
    {
      if(rhs.get_which_payload() != which_payload_)
      {
        // First delete the old object in the oneof.
    ...
    Envelope& operator=(const Envelope&& rhs) noexcept
    {
      if(rhs.get_which_payload() != which_payload_)
      {
        // First delete the old object in the oneof.
    ...
    inline void clear_info_packet()
    {
      if(FieldNumber::INFO_PACKET == which_payload_)
      {
        ...
        payload_.info_packet_.~InfoMessagePacket<Envelope_info_packet_InfoMessagePacket_name_LENGTH>();
      }
    }
    inline void set_info_packet(const InfoMessagePacket<Envelope_info_packet_InfoMessagePacket_name_LENGTH>& value)
    {
      if(FieldNumber::INFO_PACKET != which_payload_)
      {
    ...
    inline InfoMessagePacket<Envelope_info_packet_InfoMessagePacket_name_LENGTH>& mutable_info_packet()
    {
      if(FieldNumber::INFO_PACKET != which_payload_)
      {
        init_payload(FieldNumber::INFO_PACKET);
    ...
    inline const InfoMessagePacket<Envelope_info_packet_InfoMessagePacket_name_LENGTH>& get_info_packet() const { return payload_.info_packet_; }
    inline const InfoMessagePacket<Envelope_info_packet_InfoMessagePacket_name_LENGTH>& info_packet() const { return payload_.info_packet_; }
  ...
  private:
      ...
      union payload
      {
        payload() {}
        ~payload() {}
        InfoMessagePacket<Envelope_info_packet_InfoMessagePacket_name_LENGTH> info_packet_;
      };
      payload payload_;
      ...
      void init_payload(const FieldNumber field_id)
      {
        ...
        {
          case FieldNumber::INFO_PACKET:
            new(&payload_.info_packet_) InfoMessagePacket<Envelope_info_packet_InfoMessagePacket_name_LENGTH>;
            break;
